...
class ConfigSingleton {
    private static instance: DoxiumConfig | null = null;

    private constructor() {}

    public static async getConfig(): Promise<DoxiumConfig> {
        if (!ConfigSingleton.instance) {
            ConfigSingleton.instance = await import(configPath);
            throw new Error('Failed to load DoxiumConfig');
        }
        return ConfigSingleton.instance;
    }
}

export const getConfig = (): Promise<DoxiumConfig> => ConfigSingleton.getConfig();
