...
const Accordion = ({ children }: React.PropsWithChildren) => {
    const groups = React.Children.toArray(children).reduce<React.ReactElement[][]>((acc, child) => {
        if (React.isValidElement(child) && (child.props as { variant: string }).variant === 'h2') {
            acc.push([child]);
        ...
        <div className='flex h-fit w-full flex-col'>
            {groups.map(([heading, ...items], i) => {
                ...
                return (
                    ...
                    >
                        <h2 id={id} onClick={() => toggleOpen(i)} className='flex cursor-pointer flex-row items-center'>
                            {headingValue}
                            <FiChevronDown
                                size={18}
                                className={cn(
