...
const Alert = ({ type = 'accent', children, link, description }: React.PropsWithChildren<AlertProps>) => {
    return link ? (
        <a
            className={cn(
                'not-prose my-3 flex w-full flex-row gap-1 items-center rounded-lg border-[0.01em] px-3.5 py-2.5 text-sm font-normal',
                getAlertColor(type),
            )}
            href={link}
            target='_blank'
        ...
        <span
            className={cn(
                'not-prose my-3 flex w-full gap-1 flex-row items-center rounded-lg border-[0.01em] px-3.5 py-2.5 text-sm font-normal',
                getAlertColor(type),
            )}
        >
            {getAlertIcon(type)}
